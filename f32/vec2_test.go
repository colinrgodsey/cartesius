...
func TestVec2(t *testing.T) {
	...
	if !a.Add(b).Eq(NewVec2(1, 3, 5, 7)) {
		t.Fatal("add failed")
	}
	if !a.Sub(b).Eq(one) {
		t.Fatal("add failed")
	}
	if NewVec2(0, 1, 0, 0).Mag() != 1.0 {
		t.Fatalf("dot failed")
	}
	if !one.Inv().Eq(one) {
		t.Fatalf("inv failed")
	...
	if absfloat32(NewVec2(2, 2, 2, 2).Mul(100).Unit().Mag()-1.0) > 1e-6 {
		fmt.Println(NewVec2(2, 2, 2, 2).Mul(100).Unit().Mag())
